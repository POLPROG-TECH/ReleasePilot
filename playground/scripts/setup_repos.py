...
ACME_WEB_COMMITS = [
    ...
    (75, "feat(dashboard): implement real-time analytics widget", {"src/dashboard.py": "# Analytics\n"}),
    ...
    (30, "security: patch XSS vulnerability in comment rendering", {"src/comments.py": "# Patched\n"}),
...
ORBIT_MOBILE_COMMITS = [
    ...
    (5, "feat(accessibility): add VoiceOver support for all views", {"src/a11y.swift": "// A11y\n"}),
...
PULSE_CLI_COMMITS = [
    ...
    (40, "feat!: change default output format from JSON to YAML", {"src/output.rs": "// YAML default\n"}),
    ...
    (10, "improvement: add colored terminal output with auto-detect", {"src/color.rs": "// Color\n"}),
    (7, "fix: correct exit code for partial failures", {"src/exit.rs": "// Exit code\n"}),
    (5, "feat: add shell completion generation (bash/zsh/fish)", {"src/complete.rs": "// Completion\n"}),
...
SPARK_SAAS_COMMITS = [
    ...
    (76, "fix(billing): correct proration calculation for upgrades", {"src/prorate.ts": "// Prorate\n"}),
    ...
    (58, "feat(api): add GraphQL subscription for live updates", {"src/subscriptions.ts": "// Live\n"}),
    (52, "improvement(perf): optimize database connection pooling", {"src/pool.ts": "// Pool\n"}),
    (46, "feat(export): add data export in CSV and Excel formats", {"src/export.ts": "// Export\n"}),
    (40, "fix(auth): resolve SSO session timeout handling", {"src/sso.ts": "// SSO\n"}),
    (34, "feat(integrations): add Slack and Teams notifications", {"src/integrations.ts": "// Slack\n"}),
    ...
    (16, "fix(webhook): add retry logic with exponential backoff", {"src/webhook.ts": "// Retry\n"}),
...
ATLAS_B_COMMITS = [
    ...
    (8, "improvement: add order tracking webhook notifications", {"src/tracking.js": "// Tracking\n"}),
...
def _create_repo(name: str, commits: list[tuple], *, tags: list[tuple] | None = None,
    ...
    env = {"GIT_AUTHOR_NAME": "Demo User", "GIT_AUTHOR_EMAIL": "demo@example.com",
    ...
    for days_ago, message, files in commits:
        ...
        if days_ago in tag_map:
            _run(["git", "tag", "-a", tag_map[days_ago], "-m",
                  f"Release {tag_map[days_ago]}"], str(repo), env)
...
def main() -> None:
    ...
    repos = [
        ("acme-web", ACME_WEB_COMMITS, ACME_WEB_TAGS, None,
         "Web app — tags, mixed commit types, 85-day history"),
        ("nova-api", NOVA_API_COMMITS, None, None,
         "API service — conventional commits, no tags, 60-day history"),
        ("orbit-mobile", ORBIT_MOBILE_COMMITS, None, ORBIT_MOBILE_CHANGELOG,
         "Mobile app — existing CHANGELOG.md"),
        ("pulse-cli", PULSE_CLI_COMMITS, None, None,
         "CLI tool — security fixes, breaking changes"),
        ("spark-saas", SPARK_SAAS_COMMITS, None, None,
         "SaaS platform — executive/customer demo, 88-day history"),
        ("atlas-multi-a", ATLAS_A_COMMITS, None, None,
         "Microservice A — user service (multi-repo demo)"),
        ("atlas-multi-b", ATLAS_B_COMMITS, None, None,
